...
```bash
# .env 計費設定
FREE_CREDITS=100              # 註冊免費額度
CREDITS_PER_PURCHASE=100      # 每次儲值獲得額度
PRICE_PER_PURCHASE=0.01       # 每次儲值價格 (USD)
AUTHOR_SHARE=0.90             # 作者分潤比例 (90%)
PLATFORM_SHARE=0.10           # 平台分潤比例 (10%)
```
...
| 項目 | 金額 | 說明 |
|------|------|------|
| 每次召喚消耗 | 1 credit | 每次召喚扣 1 額度 |
| 每 credit 價值 | $0.0001 USD | PRICE_PER_PURCHASE / CREDITS_PER_PURCHASE |
